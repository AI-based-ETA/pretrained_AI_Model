...
def main():
    ...
    model.to(device)
    model.load_state_dict(torch.load(args.checkpoint, map_location=device)) # map_location=device
    model.eval()
    ...
    print("done 2")
    y12 = realy[:,99,11].to(device).detach().numpy()
    yhat12 = scaler.inverse_transform(yhat[:,99,11]).to(device).detach().numpy()
    
    y3 = realy[:,99,2].to(device).detach().numpy()
    yhat3 = scaler.inverse_transform(yhat[:,99,2]).to(device).detach().numpy()
